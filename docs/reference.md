...
문제를 입력하세요: 큐(Queue)의 특징으로 옳은 것은?
선택지 1: 먼저 들어간 데이터가 먼저 나온다 (FIFO)
선택지 2: 나중에 들어간 데이터가 먼저 나온다 (LIFO)
선택지 3: 항상 가장 큰 값이 먼저 나온다
선택지 4: 순서 없이 나온다
정답 번호 (1-4): 1
...
퀴즈 목록(예시)
    ...
    ----------------------------------------
    [1] 스택(Stack)의 자료 처리 방식은?
    [2] 파이썬의 딕셔너리(dict)가 내부적으로 사용하는 자료구조는?
    ...
    ----------------------------------------
