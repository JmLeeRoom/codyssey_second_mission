...
| 주제 예시 | 좋은 이유 |
|---|---|
| 영화 / 드라마 | 4지선다 문제 만들기 쉬움, 정답 논란 적음 |
| K-POP / 아이돌 | 흥미 유지에 좋음 |
| 프로그래밍 기초 용어 | 배운 내용 복습 효과 |
| 세계사 / 지리 | 정답이 명확함 |
| 내가 다니는 학교/동네 | 나만의 개성 |

> [!TIP]
> **정답이 명확하게 하나로 떨어지는 주제**를 고르세요. "가장 좋은 영화는?" 같은 주관적 문제는 피합니다.
...
> ```python
> quiz = Quiz("Python의 창시자는?", ["Guido", "Linus", "Bjarne", "James"], 1)
> quiz.choices[quiz.answer - 1]   # → "Guido"   ← 꺼낼 때만 -1
> quiz.is_correct(1)              # → True
> ```
...
Step 0에서 정한 주제로 **5개 이상**의 퀴즈를 만드는 함수를 작성하세요.

```python
def get_default_quizzes():
    """파일이 없을 때(첫 실행) 사용할 기본 퀴즈 목록을 반환한다."""
    return [
        Quiz("마블 시네마틱 유니버스에서 타노스가 모은 인피니티 스톤의 개수는?",
             ["4개", "5개", "6개", "7개"], 3),
        # TODO: 여기에 내 주제의 퀴즈를 4개 이상 더 추가
    ]
```
...
```bash
git add main.py
git commit -m "Feat: 영화 주제 기본 퀴즈 데이터 5개 추가"
git push
```
...
| 3 | `Feat: 영화 주제 기본 퀴즈 데이터 5개 추가` | 1-3 직후 |
...
> | `ensure_ascii=False` | `"타노스"` 가 `"\ud0c0\ub178\uc2a4"` 로 저장되어 사람이 못 읽음 (JSON으로는 유효하지만 열어봐도 알아볼 수 없음) |
>
...
## 2. 퀴즈 주제와 선정 이유
- **주제**: 영화
- **선정 이유**: 정답이 명확해 4지선다로 만들기 좋고, 평소 관심 있는 분야라
  문제를 직접 만들면서 흥미를 유지할 수 있었습니다.
...
| 3 | 1 | `Feat: 영화 주제 기본 퀴즈 데이터 5개 추가` | main | `add` `commit` `push` |
