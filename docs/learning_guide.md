# 🎯 자료구조 터미널 퀴즈 게임 만들기 — 학습 및 실습 가이드

> 자료구조(Data Structure)를 주제로 Python 기초 · 클래스(OOP) · 파일 입출력(JSON) · Git/GitHub 워크플로우를
...
터미널(콘솔)에서 동작하는 **자료구조(Data Structure) 4지선다 퀴즈 게임**을 만듭니다.
...
| 제출물 | 만족해야 할 조건 |
|---|---|
| **Python 콘솔 프로그램 1개** | 메뉴 5종 동작 · 최소 2개 클래스(`Quiz`, `QuizGame`) · 자료구조 퀴즈 5개 이상 · 프로그램을 껐다 켜도 데이터 유지(`state.json`) |
...
```
![Python 및 Git 개발 환경 설정](image.png)
> [!IMPORTANT]
...
README에 **"주제 선정 이유"** 를 써야 하므로, 코딩 전에 주제를 분명히 정합니다. 이 가이드는 퀴즈 주제를 **자료구조(Data Structure)** 로 통일합니다.

> [!NOTE]
> **왜 자료구조인가요?**
> - 스택의 LIFO, 큐의 FIFO처럼 정답이 하나로 명확해 4지선다에 적합합니다.
> - 게임 안에서도 퀴즈 목록은 `list`, 저장 데이터는 `dict`와 JSON으로 다루므로 주제와 구현이 연결됩니다.
> - 스택·큐·해시 테이블·트리·시간 복잡도는 언어가 바뀌어도 계속 쓰이는 기초 컴퓨터 과학 지식입니다.

출제할 만한 세부 영역입니다. 아래에서 **5개 이상**을 골라 문제를 만드세요.

| 세부 영역 | 물어보기 좋은 것 |
|---|---|
| 스택 / 큐 | LIFO·FIFO 동작 방식, 함수 호출 스택, 대기열 |
| 배열 / 연결 리스트 | 임의 접근과 중간 삽입·삭제의 차이 |
| 해시 테이블 | 해시 함수의 역할, 평균 탐색 시간 복잡도 |
| 트리 / 이진 탐색 트리 | 루트·리프 용어, 왼쪽·오른쪽 서브트리의 규칙 |
| 시간 복잡도 | O(1), O(log n), O(n)의 의미와 대표 연산 |

> [!TIP]
> **정답이 명확하게 하나로 떨어지는 문제**를 만드세요. "가장 좋은 자료구조는?"처럼 조건에 따라 답이 달라지는 질문은 피합니다.
> 그리고 지금 메모장에 **문제 5개 + 선택지 4개씩 + 정답 번호**를 미리 적어두면 Step 1이 순식간에 끝납니다.
...
## 1. 프로젝트 개요
자료구조를 주제로 한 터미널 4지선다 퀴즈 게임입니다. 퀴즈를 풀고 직접 문제를 추가하며,
최고 점수를 `state.json`에 저장해 프로그램을 다시 실행해도 유지합니다.

## 2. 퀴즈 주제와 선정 이유
- **주제**: 자료구조 (Data Structure)
- **선정 이유**: 정답이 명확해 4지선다에 적합하고, 프로그램에서 사용하는 `list`·`dict`와 연결해
  핵심 컴퓨터 과학 개념을 함께 복습할 수 있습니다.
...
> ```python
> quiz = Quiz("스택(Stack)의 자료 처리 방식은?", ["FIFO", "LIFO", "우선순위 순", "무작위 접근"], 2)
> quiz.choices[quiz.answer - 1]   # → "LIFO"   ← 꺼낼 때만 -1
...
자료구조 핵심 개념을 주제로 **5개 이상**의 퀴즈를 만드는 함수를 작성하세요.
...
def get_default_quizzes():
    ...
    return [
        ...
        Quiz("큐(Queue)의 자료 처리 방식은?",
             ["LIFO (후입선출)", "FIFO (선입선출)", "재귀 호출", "해시 탐색"], 2),
        Quiz("해시 테이블에서 키를 저장 위치로 바꾸는 함수는?",
             ["정렬 함수", "해시 함수", "재귀 함수", "탐색 함수"], 2),
        Quiz("이진 탐색 트리(BST)에서 왼쪽 서브트리의 키는 루트와 어떤 관계인가요?",
             ["루트보다 작다", "루트보다 크다", "항상 같다", "관계없다"], 1),
        Quiz("파이썬 리스트에서 인덱스로 원소 하나에 접근하는 시간 복잡도는?",
...
if __name__ == "__main__":
    ...
    quizzes[0].display(1)
    print(quizzes[0].is_correct(2))   # 정답 번호를 넣으면 True
...
- [ ] 자료구조 퀴즈가 **5개 이상**이고, 각각 선택지가 정확히 4개다
...
> | `ensure_ascii=False` | `"스택"` 이 유니코드 이스케이프 형태로 저장되어 사람이 읽기 어려움 (JSON으로는 유효하지만 열어봐도 알아보기 어려움) |
>
...
- **주제**: 자료구조 (Data Structure)
- **선정 이유**
  1. 스택의 LIFO, 큐의 FIFO처럼 정답이 명확해 4지선다에 적합합니다.
  2. 퀴즈 목록의 `list`, 저장 데이터의 `dict`처럼 프로그램 내부 구조와 주제가 연결됩니다.
  3. 스택·큐·해시 테이블·트리는 오래 활용되는 기초 컴퓨터 과학 지식입니다.
